...
const options = [
    ...
    { key: "Successful Upgrade", values: [{"direction": ["UPGRADE"]}, {"request_status": ["COMPLETED"]}]},
    { key: "Aborted Upgrade", values: [{"direction": ["UPGRADE"]}, {"request_status": ["ABORTED"]}]},
    { key: "Failed Upgrade", values: [{"direction": ["UPGRADE"]}, {"request_status": ["FAILED"]}]},
    { key: "Successful Downgrade", values: [{"direction": ["DOWNGRADE"]}, {"request_status": ["COMPLETED"]}]},
    { key: "Aborted Downgrade", values: [{"direction": ["DOWNGRADE"]}, {"request_status": ["ABORTED"]}]},
    { key: "Failed Downgrade", values: [{"direction": ["DOWNGRADE"]}, {"request_status": ["FAILED"]}]},
  ];
...
export default function UpgradeHistory() {
    ...
    useEffect(() => {
        if(selectedOption.key !== "All") {
            setUpgrades(originalUpgrades.filter((upgrade) => {
    ...
    const columns = [
            ...
            cell : (info: any) => {
                ...
                return (
                    <>
                        <div>
                            { isExpanded ? (
                                <FontAwesomeIcon icon={faCaretDown} onClick={() => toggleExpand()} />
                            ) : (
                                <FontAwesomeIcon icon={faCaretRight} onClick={() => toggleExpand()} />
                            )}
                            <Button variant="link" size="sm" onClick={() => handleOpenUpgrade()}>{get(info, "row.original.Upgrade.direction")}</Button>
                        </div>
                        ...
                        { isExpanded && (
                            <div className="mt-2 full-width">
                                <table className="table">
                                    <tbody>
                                        {Object.entries(versions).map(([serviceName, versionInfo]) => (
                                            <tr key={serviceName}>
        ...
        },
        {
            ...
            cell: (info: any) => {
                return (
                    getUpgradeDisplayName(get(info, "row.original.Upgrade.upgrade_type"))
                )
            }
        },
        {
            ...
            cell: (info: any) => {
                return (
                    get(info, "row.original.Upgrade.associated_version")
                )
            }
        },
        {
            ...
            cell: (info: any) => {
                return (
                    getUpgradeType(get(info, "row.original.Upgrade"))
                )
            }
        },
        {
            ...
            width: "15%",
            cell: (info: any) => {
                const startTime = new Date(get(info, "row.original.Upgrade.start_time"));
                return startTime.toDateString() + " " + startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
            },
        },
        {
            ...
            cell: (info: any) => {
                ...
                const duration = calculateDurationSummary(start_time, end_time);
        ...
        },
        {
            ...
            cell: (info: any) => {
                // START GENAI
                const endTimeValue = get(info, "row.original.Upgrade.end_time");
                
                if (!endTimeValue || endTimeValue < 1) {
                    return "Not finished";
                }
                
                const endTime = new Date(endTimeValue);
                
                if (endTime.getFullYear() === 1969 || isNaN(endTime.getTime())) {
                    return "Not finished";
                }
                
                return endTime.toDateString() + " " + endTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                // END GENAI
            },
        },
        {
            ...
            cell: (info: any) => {
                return (
                    get(info, "row.original.Upgrade.request_status")
                )
            }
        },
    ]

    // Check authorization first - if user doesn't have permission, show unauthorized message
    if (!canViewUpgradeHistory) {
        return (
            <div className="text-left mt-5">
                <h2>Access Denied</h2>
                <p>You do not have permission to view upgrade history. Required permission: CLUSTER.UPGRADE_DOWNGRADE_STACK</p>
            </div>
        );
    }
    ...
    return (
        <>
            ...
            <div className="mt-5">
                ...
                <Table columns={columns} data={upgrades}>
                </Table>
            </div>
        </>
    )
