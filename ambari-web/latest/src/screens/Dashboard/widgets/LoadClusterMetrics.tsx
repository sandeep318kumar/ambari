...
interface LoadClusterMetricsProps {
  timeRange?: string;
  customTimeRange?: {
    startTime: number;
    endTime: number;
...
const LoadClusterMetrics: React.FC<LoadClusterMetricsProps> = ({ timeRange = "Last 1 hour", customTimeRange = null }) => {
  ...
  const fetchLoadMetrics = async () => {
    ...
    try {
      ...
      const fields = [
        "metrics/load/1-min._avg",
        "metrics/load/CPUs._avg",
        "metrics/load/Nodes._avg",
        "metrics/load/Procs._avg"
      ].join(",");
      ...
      if (response.metrics && response.metrics.load) {
        setLoadData(response.metrics.load);
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Failed to fetch load metrics";
  ...
  if (error) {
    return (
      <Alert variant="danger" className="m-2">
        Error loading load metrics: {error}
      </Alert>
    );
  }
  ...
  if (isEmpty(loadData)) {
    return (
      <Alert variant="info" className="m-2">
        No load metrics data available
      </Alert>
    );
  }
  ...
  if (loadData["1-min._avg"]) {
    seriesData.push({
      name: "1 Minute Load",
      data: loadData["1-min._avg"],
      color: "#FF8000",
    });
  }
  
  if (loadData["CPUs._avg"]) {
    seriesData.push({
      name: "CPUs Load",
      data: loadData["CPUs._avg"],
      color: "#0066B3",
    });
  }
  
  if (loadData["Nodes._avg"]) {
    seriesData.push({
      name: "Nodes Load",
      data: loadData["Nodes._avg"],
      color: "#00CC00",
    });
  }
  
  if (loadData["Procs._avg"]) {
    seriesData.push({
      name: "Processes Load",
      data: loadData["Procs._avg"],
      color: "#FFCC00",
    });
  }
...
export default LoadClusterMetrics;
