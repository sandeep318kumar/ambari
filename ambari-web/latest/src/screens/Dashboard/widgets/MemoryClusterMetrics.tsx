...
interface MemoryClusterMetricsProps {
  timeRange?: string;
  customTimeRange?: {
    startTime: number;
    endTime: number;
...
const MemoryClusterMetrics: React.FC<MemoryClusterMetricsProps> = ({ timeRange = "Last 1 hour", customTimeRange = null }) => {
  ...
  const fetchMemoryMetrics = async () => {
    ...
    try {
      ...
      const fields = [
        "metrics/memory/Buffer._avg",
        "metrics/memory/Cache._avg", 
        "metrics/memory/Share._avg",
        "metrics/memory/Swap._avg",
        "metrics/memory/Total._avg"
      ].join(",");
      ...
      if (response.metrics && response.metrics.memory) {
        setMemoryData(response.metrics.memory);
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Failed to fetch memory metrics";
  ...
  if (memoryData["Buffer._avg"]) {
    seriesData.push({
      name: "Buffer Memory",
      data: memoryData["Buffer._avg"].map(([value, timestamp]: [number, number]) => [
        value / Math.pow(2, 20), // Convert to MB
        timestamp,
      ]),
      color: "#0066B3",
    });
  ...
  if (memoryData["Cache._avg"]) {
    seriesData.push({
      name: "Cache Memory",
      data: memoryData["Cache._avg"].map(([value, timestamp]: [number, number]) => [
        value / Math.pow(2, 20), // Convert to MB
        timestamp,
      ]),
      color: "#00CC00",
  ...
  if (memoryData["Share._avg"]) {
    seriesData.push({
      name: "Shared Memory",
      data: memoryData["Share._avg"].map(([value, timestamp]: [number, number]) => [
        value / Math.pow(2, 20), // Convert to MB
        timestamp,
      ]),
      color: "#FF8000",
  ...
  if (memoryData["Swap._avg"]) {
    seriesData.push({
      name: "Swap Memory",
      data: memoryData["Swap._avg"].map(([value, timestamp]: [number, number]) => [
        value / Math.pow(2, 20), // Convert to MB
        timestamp,
      ]),
      color: "#FFCC00",
  ...
  if (memoryData["Total._avg"]) {
    seriesData.push({
      name: "Total Memory",
      data: memoryData["Total._avg"].map(([value, timestamp]: [number, number]) => [
        value / Math.pow(2, 20), // Convert to MB
        timestamp,
      ]),
      color: "#800080",
...
export default MemoryClusterMetrics;
